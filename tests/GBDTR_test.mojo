...
def main() raises:
    var gbdtr_test = Python.import_module("load_boston")
    var data = gbdtr_test.get_data() # X, y
    ...
    var gbdtr = GBDT(criterion='mse', n_trees = 10, max_depth = 6, learning_rate = 0.3, reg_lambda = 0.5, gamma = 0.1)
    gbdtr.fit(X_train, y_train)
    gbdtr.save('gbdtr')
    gbdtr = GBDT.load('gbdtr')
    var y_pred = gbdtr.predict(X_test)
    print("GBDT regression MSE:", mse(y_test, y_pred))
    print("GBDT regression Accuracy:", r2_score(y_test, y_pred))
    os.remove('gbdtr.mjml')
