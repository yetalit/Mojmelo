...
def main() raises:
    ...
    gbdtr = GBDT[criterion='mse'](n_trees = 10, max_depth = 6, learning_rate = 0.3, reg_lambda = 0.5, gamma = 0.1)
    gbdtr.fit(X_train, y_train)
    gbdtr.save('gbdtr')
    gbdtr_load = GBDT.load[0]('gbdtr')
    y_pred = gbdtr_load.predict(X_test)
