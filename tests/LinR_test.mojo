...
def main() raises:
    var lr_test = Python.import_module("LinR_test")
    ...
    var lr = LinearRegression(0.01, 1000)
    lr.fit(X_train, y_train)
    lr.save('lnr')
    lr = LinearRegression.load('lnr')
    var y_pred = lr.predict(X_test)
    print("MSE:", mse(y_test, y_pred))
    print("Accuracy:", r2_score(y_test, y_pred))
    var y_pred_line = lr.predict(Matrix.from_numpy(data[0]))
