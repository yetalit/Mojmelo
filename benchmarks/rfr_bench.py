...
def prepare_data(seed, n_samples=100000, n_features=50, test_ratio=0.2):
    X, y = make_regression(
        n_samples=n_samples,
...
def sklearn_model():
    return RandomForestRegressor(n_estimators=10, max_depth=10, random_state=42, n_jobs=-1)


# -----------------------
# HIGH-LEVEL RUNNER
# -----------------------
def run_benchmark(seed, fit_mean, fit_std, pred_mean, pred_std, mse):
    ...
    print("| Model       | Fit Time (s)    | Predict Time (s) | MSE       |")
    print("|-------------|-----------------|------------------|-----------|")

    print(
        f"| sklearn RFR | {sk['fit_mean']:.4f} ± {sk['fit_std']:.4f} "
        f"| {sk['pred_mean']:.4f} ± {sk['pred_std']:.4f}  "
        f"| {sk['mse']:.4f} |"
    )

    print(
        f"| mojmelo RFR | {fit_mean:.4f} ± {fit_std:.4f} "
