...
def sklearn_model():
    return RandomForestClassifier(n_estimators=10, max_depth=10, random_state=42, n_jobs=-1)
...
def run_benchmark(seed, fit_mean, fit_std, pred_mean, pred_std, accuracy):
    ...
    print(
        f"| sklearn RFC | {sk['fit_mean']:.4f} ± {sk['fit_std']:.4f} "
        f"| {sk['pred_mean']:.4f} ± {sk['pred_std']:.4f}  "
        f"| {sk['accuracy']:.4f}   |"
    )

    print(
        f"| mojmelo RFC | {fit_mean:.4f} ± {fit_std:.4f} "
