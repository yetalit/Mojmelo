...
def main() raises:
    ...
    var rfc_bench = Python.import_module("rfc_bench")
    var data = rfc_bench.prepare_data(GLOBAL_SEED) # X_train, X_test, y_train, y_test
    var X_train, X_test, y_train, y_test = Matrix.from_numpy(data[0]), Matrix.from_numpy(data[1]), Matrix.from_numpy(data[2]).T(), Matrix.from_numpy(data[3]).T()
    ...
    for _ in range(WARMUP):
        var rfc = RandomForest(n_trees=10, max_depth=10)
        rfc.fit(X_train, y_train)
        var y_pred = rfc.predict(X_test)
    ...
    rfc_bench.run_benchmark(GLOBAL_SEED, fit_mean, fit_std, pred_mean, pred_std, accuracy)
