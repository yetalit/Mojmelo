...
def prepare_data(seed, n_samples=300000, n_features=150):
...
def sklearn_model():
    return KMeans(n_clusters=5, max_iter=100, random_state=42)
...
def run_benchmark(seed, mean, std, labels):
    ...
    print(
        f"| sklearn KMeans | {sk['mean']:.4f} ± {sk['std']:.4f} "
        f"| {'-':^14} "
        f"| {sk_ari_vs_truth:.4f}       |"
    )

    print(
        f"| mojmelo KMeans | {mean:.4f} ± {std:.4f} "
        f"| {ari_vs_sklearn:.4f}         "
        f"| {ari_vs_truth:.4f}       |"
    )
