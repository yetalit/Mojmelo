from mojmelo.DecisionTree import DecisionTree
from mojmelo.utils.Matrix import Matrix
from mojmelo.utils.utils import accuracy_score
from std.python import Python
import std.random as random
from std.benchmark import keep
import std.time as time

def main() raises:
    random.seed()
    var GLOBAL_SEED = random.random_si64(0, 1_000_000)

    var dtc_bench = Python.import_module("dtc_bench")
    var data = dtc_bench.prepare_data(GLOBAL_SEED) # X_train, X_test, y_train, y_test
    var X_train, X_test, y_train, y_test = Matrix.from_numpy(data[0]), Matrix.from_numpy(data[1]), Matrix.from_numpy(data[2]).T(), Matrix.from_numpy(data[3]).T()

    var WARMUP = 2
    var RUNS = 5
    var accuracy = 0.0
    # warm-up
    for _ in range(WARMUP):
        var dtc = DecisionTree(max_depth=10)
        dtc.fit(X_train, y_train)
        var y_pred = dtc.predict(X_test)
        accuracy = accuracy_score(y_test, y_pred).cast[DType.float64]()

    var fit_times: List[Float64] = []
    var pred_times: List[Float64] = []

    # timed runs
    for _ in range(RUNS):
        var dtc = DecisionTree(max_depth=10)

        var t0 = time.perf_counter()
        dtc.fit(X_train, y_train)
        var t1 = time.perf_counter()

        var t2 = time.perf_counter()
        keep(dtc.predict(X_test))
        var t3 = time.perf_counter()

        fit_times.append(t1 - t0)
        pred_times.append(t3 - t2)

    var fit_sum = 0.0
    var pred_sum = 0.0

    for i in range(RUNS):
        fit_sum += fit_times[i]
        pred_sum += pred_times[i]

    var fit_mean = fit_sum / Float64(RUNS)
    var pred_mean = pred_sum / Float64(RUNS)

    var fit_var = 0.0
    var pred_var = 0.0

    for i in range(RUNS):
        fit_var += (fit_times[i] - fit_mean) ** 2
        pred_var += (pred_times[i] - pred_mean) ** 2

    var fit_std = (fit_var / Float64(RUNS)) ** 0.5
    var pred_std = (pred_var / Float64(RUNS)) ** 0.5

    dtc_bench.run_benchmark(GLOBAL_SEED, fit_mean, fit_std, pred_mean, pred_std, accuracy)
