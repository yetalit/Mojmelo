...
def micro_kernel[
    ...
    comptime assert nr % simd_width == 0, "nr must be a multiple of simd_width"
    ...
    var cr_base = stack_allocation[mr * nr, Scalar[Type], alignment=64]()

    # Load C tile into cr_base
    comptime if padding:
...
def loop_n[
    ...
    var nc_actual = roundup(min(nc, roundup((N + n_threads - 1) // n_threads, nr)), nr)
    nc_actual = max(nc_actual, nr)
    ...
    def process_tile(tile_idx: Int):
        ...
        var Bc_buffer = UnsafePointer[Scalar[Type], MutAnyOrigin](
            _malloc[Scalar[Type]](kc * nc_actual * size_of[Type](), alignment=64).value()
        )
...
def matmul_impl[
    ...
    var Ac_buffer = _malloc[Scalar[Type]](mc * kc * size_of[Type](), alignment=64)
...
def matmul_params[Type: DType]() -> IndexList[5]:
    ...
    comptime if Type.is_floating_point():
        return compute_params[Vectors - 1]()
    else:
        comptime if Type == DType.int64:
            comptime if CompilationTarget.has_avx512f():
                return compute_params[Vectors - 2]()
            else:
                return compute_params[Vectors - 3]()
        else:
            return compute_params[Vectors - 2]()
