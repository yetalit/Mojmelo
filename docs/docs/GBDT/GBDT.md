...
- `MODEL_ID = 11`
- `criterion_ids = List(String("log"), String("softmax"), String("mse"), __list_literal__=NoneType(None))`
...
- **loss_g** (`def(Matrix, Matrix) raises thin -> Matrix`)
- **loss_h** (`def(Matrix) raises thin -> Matrix`)
...
- **trees** (`Pointer[BDecisionTree, MutUntrackedOrigin]`)
- **score_start** (`Float32`)
- **num_class** (`Int`)
...
```mojo
fn def __init__(out self, criterion: String = "log", n_trees: Int = Int(10), min_samples_split: Int = Int(10), max_depth: Int = Int(3), learning_rate: Float32 = 0.10000000000000001, reg_lambda: Float32 = 1, reg_alpha: Float32 = 0, gamma: Float32 = 0, n_bins: Int = Int(0))
```
