...
struct PCA:
    ...
    fn __init__(out self, n_components: Int, whiten: Bool = False):
    ...
    fn fit(mut self, X: Matrix) raises:
        ...
        var indices = List[Int](capacity=S.size)
        indices.resize(S.size, 0)
        ...
        @parameter
        fn p(i: Int):
            self.components[i, unsafe=True] = Vt[indices[i], unsafe=True]
    ...
    fn transform(self, X: Matrix) raises -> Matrix:
        # project data
        if self.whiten:
            return ((X - self.mean) * self.components.T()) / self.whiten_
